...
#### Note: The function tapply() is used to apply a function, here mean(), to each group of components of the first argument, here incomes, defined by the levels of the second component, here statef2

#### 2 that tapply() also works in this case when its second argument is not a factor, 

#### e.g., ‘tapply(incomes, state)’, and this is true for quite a few other functions, since arguments are coerced to factors when necessary (using as.factor()).
...
```{r,}
Breast_cancer_xlsx <- read_excel("/cloud/project/RWorksheet_JACULINA#7a/DATA_BREAST_CANCER.xlsx")
Breast_cancer_xlsx
```
